...
function sgs.ai_armor_value.Vine(player, self)
	...
	if self:hasSkills("mengwu|chiyan|wunv",player) then return 6 end
	if player:hasSkill("lingti") then return -1 end
	for _, enemy in ipairs(self:getEnemies(player)) do
		if (enemy:canSlash(player) and self:isEquip("Fan",enemy)) or self:hasSkills("huoji|shaoying|zonghuo|wuling|chiyan|rexue|spwushen", enemy)
...
function SmartAI:searchForAnaleptic(use,enemy,slash)
	...
	if self.player:hasSkill("haoyin") then return nil end
...
function SmartAI:useCardIronChain(card, use)	
	...
	local mamoru = self.room:findPlayerBySkillName("luren")
	self:sort(self.friends,"defense")
	for _, friend in ipairs(self.friends) do
		if friend:isChained() and not self:isGoodChainPartner(friend) and self:hasTrickEffective(card, friend) and not self:hasSkills("danlao|luren",friend) then
			table.insert(friendtargets, friend)
		end
	end
	self:sort(self.enemies,"defense")
	for _, enemy in ipairs(self.enemies) do
		if not enemy:isChained() and not self.room:isProhibited(self.player, enemy, card) and not self:hasSkills("danlao|luren",enemy)
	...
	if not self.player:hasSkill("noswuyan") then
		...
		elseif #friendtargets == 1 then
			...
			elseif mamoru and self:isFriend(mamoru) then
				if use.to then use.to:append(friendtargets[1]) end
				if use.to then use.to:append(mamoru) end			
