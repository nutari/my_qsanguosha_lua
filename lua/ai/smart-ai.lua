...
function setInitialTables()
	...
	sgs.need_kongcheng = 		"lianying|kongcheng|beifa|dianxing"
	sgs.masochism_skill = 		"sinve|yiji|fankui|jieming|neoganglie|ganglie|enyuan|fangzhu|nosenyuan|langgu|guixin|quanji|menghuan|shijie"
	sgs.wizard_skill = 		"tianyun|xiling|guicai|guidao|jilve|tiandu|luoying|zhenlie|huanshi|yujian"
	sgs.wizard_harm_skill = 	"tianyun|guicai|guidao|jilve|yujian"
	...
	sgs.exclusive_skill = 		"spwuhun|huilei|duanchang|wuhun|buqu|jincui"
	...
	sgs.recover_skill =		"rende|kuanggu|zaiqi|jieyin|qingnang|yinghun|hunzi|shenzhi|longhun|miji|zishou|ganlu|xueji|shangshi|" ..
						"nosshangshi|chengxiang|buqu|tongxin|yuliao|huiguang"
...
function sgs.getDefense(player)
	...
	if player:hasSkill("fei") or player:hasSkill("shenge") or player:hasSkill("soul") then
		defense = defense + 10
	end	
	...
	if player:hasSkill("zuiqiang") and player:getHandcardNum()>2 then
		defense = defense + 0.6
	end	
	return defense
end
...
sgs.ai_skill_discard.gamerule = function(self, discard_num, min_num)
	...
	for i = #sortedCards, 1, -1 do		
		...
		if (self.player:hasSkill("fuyuan") and #to_discard >= least) or #to_discard >= discard_num or self.player:isKongcheng() then break end		
	end
	return to_discard
end
...
function SmartAI:askForSinglePeach(dying)
	...
	if self.player:isDead() then return "." end
	if self:immunityRecover(dying) then return "." end
	...
	if self:hasSkills("tianzhen|tianran") then return card_str or "." end
	local cards=sgs.QList2Table(self.player:getCards("he"))
	local asuka=self.room:findPlayerBySkillName("zhenya")
	if not asuka or not asuka:getPhase()==sgs.Player_Play then return card_str or "." end
	if not card_str then return "." end	
	local usecard=sgs.Card_Parse(card_str)
	local samecolor=0
	for _,cd in ipairs(cards) do
		if cd:isRed() and usecard:isRed() then samecolor=samecolor+1 end
		if cd:isBlack() and usecard:isBlack() then samecolor=samecolor+1 end
	end	
	if samecolor<=1 then return "." end
	return card_str or "."
end
...
function sgs.getSkillLists(player)
	...
	for _, askill in sgs.qlist(player:getVisibleSkillList()) do
		...
		if askill:inherits("TriggerSkill") then table.insert(vsnlist,askill:objectName()) end
	end
...
function SmartAI:canRetrial(player, to_retrial)
	...
	elseif player:hasSkill("yujian") then
...
function getCardsNum(class_name, player)
	...
	if class_name == "Slash" then
		...
		elseif player:hasSkill("xinyan") then
			slashnum = redslash + num + (player:getHandcardNum() - shownum) * 0.8
		else
			slashnum = num+(player:getHandcardNum()-shownum)*0.35
		end
		return (player:hasSkill("wushuang") or player:hasSkill("zuiqiang")) and slashnum*2 or slashnum
...
function SmartAI:aoeIsEffective(card, to, source)
	...
	if not self:damageIsEffective(to, sgs.DamageStruct_Normal, source,card,1) then
		return false
	end
...
function SmartAI:hasTrickEffective(card, player)
	if player then
		...
		if player:hasSkill("tianzhen") or player:hasSkill("tianran") then return false end
		if (player:hasSkill("zhichi") and self.room:getTag("Zhichi"):toString() == player:objectName()) or player:hasSkill("noswuyan") or (player:hasSkill("jiaoji") and not player:faceUp()) or (player:hasSkill("shengnv") and self.player:distanceTo(player)>1) or (player:getMark("@kekkai")+self.player:getMark("@kekkai")==1) then
		...
		if player:getMark("@dreamland")>0 or player:getMark("@soul")>0 then
			return false
		end
...
function IgnoreArmor(from, to)
	from = from or self.room:getCurrent()
	to = to or self.player
	if from:hasSkill("renxing") or from:hasSkill("tianran") or from:hasSkill("shenhun") then
		return true
	end	
