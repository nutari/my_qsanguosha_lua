...
function SmartAI:slashIsEffective(slash, to)
	...
	if self:getCardsNum("Analeptic")>0 then
		if not self:damageIsEffective(to, nature,self.player,slash,2) then return false end
	else
		if not self:damageIsEffective(to, nature,self.player,slash,1) then return false end
	end	
...
function SmartAI:shouldUseAnaleptic(target, slash)
	if not self:hasSkills("jueqing|chushi") then
		if self:isEquip("SilverLion", target) and not IgnoreArmor(self.player, target) then return false end
		if self:hasSkills("mengwu|bumie",target) then return false end
		if slash.nature~=sgs.DamageStruct_Normal and target:hasSkill("wunv") then return false end
		if self.player:hasSkill("huanxing") and not target:isKongcheng() then return false end
	end	
	
	if target:hasSkill("zuiqiang") then return false end
	if self:hasSkills("tianshi|shenge",target) then return true end
...
function SmartAI:useCardSlash(card, use)
	...
	if card:getSkillName() == "jihun" and (not self.player:getWeapon() or self.player:getWeapon() and self.player:getWeapon():getEffectiveId()==card:getEffectiveId()) then no_distance = true self.slash_targets=self.slash_targets+2 end
	...
	if self.player:hasSkill("create") then self.slash_targets = 1000 end
	...
	for _, friend in ipairs(self.friends_noself) do
		...
		or (self.player:hasSkill("shane") and self.player:isWounded() and friend:isWounded())
		then
	...
	for _, target in ipairs(targets) do
		...
		not (not self:isWeak(target) and #self.enemies > 1 and #self.friends > 1 and self.player:hasSkill("keji")
			and not self:isEquip("Crossbow")) then
...
sgs.ai_card_intention.Slash = function(card,from,tos)
	...
	for _, to in ipairs(tos) do
		...
		if from:hasSkill("shane") and to:isWounded() then value=0 end
...
sgs.ai_skill_cardask["slash-jink"] = function(self, data, pattern, target)
	...
	if self:isFriend(target) then
		if not target:hasSkill("jueqing") then
			...
			if target:hasSkill("shane") and not self.player:hasSkill("leiji") then return "." end
		end
	else
...
function SmartAI:useCardPeach(card, use)
	...
	if self:immunityRecover() then return end
	...
	if self.player:hasSkill("imouto") and self.player:getCards("he"):length()<=5 then return end
	local peaches = 0
...
function SmartAI:useCardIndulgence(card, use)
	...
	local getvalue = function(enemy)
		...
		if self:skipJudge(enemy,card) then return -100 end
...
function SmartAI:willUseLightning(card)
	...
	elseif not hasDangerousFriend() then
		...
		if self.player:hasSkill("paiji") then ratio=999 end
		if ratio > 1.5 then
			return true
		end
	end
