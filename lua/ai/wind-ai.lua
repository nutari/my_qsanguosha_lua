...
function sgs.ai_skill_invoke.jushou(self, data)
	if self.player:faceUp() and self.player:hasSkill("zhenggong") then return true end
...
sgs.ai_skill_use["@@leiji"]=function(self,prompt)
	...
	for _,enemy in ipairs(self.enemies) do
		if not enemy:hasArmorEffect("SilverLion") and not self:hasSkills("hongyan|rexue|chenyu|tianyun") and
			self:objectiveLevel(enemy) > 3 and not self:cantbeHurt(enemy) and not (enemy:isChained() and not self:isGoodChainTarget(enemy)) and self:damageIsEffective(enemy,sgs.DamageStruct_Thunder,self.player,nil,2) then
...
function sgs.ai_slash_prohibit.leiji(self, to, card)
	...
	if self.role == "rebel" and to:isLord() then
		...
		if not other_rebel and (self:hasSkills("hongyan|chenyu|rexue|tianyun") or self.player:getHp() >= 4) and (self:getCardsNum("Peach") > 0  or self:hasSkills("hongyan|ganglie|neoganglie")) then
			return false
		end
	end
